...
export async function GET(request: Request) {
  ...
  const payload = {
    ...
    feed_url: `${BASE_URL}/api/feed.json?page=${page}&pageSize=${pageSize}`,
    ...
    _pagination: {
      ...
      totalPages,
      next:
        page < totalPages ? `${BASE_URL}/api/feed.json?page=${page + 1}&pageSize=${pageSize}` : null,
      prev: page > 1 ? `${BASE_URL}/api/feed.json?page=${page - 1}&pageSize=${pageSize}` : null,
    },
